...
const TabContainer = (props) => {
  ...
  return (
    <>
      <div className="flex relative items-center pb-6 gap-8 md:gap-20 border-b-2 overflow-x-scroll lg:hidden">
        {tabs.map((tab) => (
          <Tab {...tab} key={`123${tab.route}`} />
        ))}
      </div>
      <div className="hidden lg:flex relative items-center pb-6 gap-8 gap-20 border-b-2">
        {tabs.map((tab) => (
          <Tab {...tab} key={`123${tab.route}`} />
        ))}
      </div>
